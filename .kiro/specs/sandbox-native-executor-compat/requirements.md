...
The main goal is to preserve a clear record of how SlideRule behaves when:
...
**User Story:** As a developer running SlideRule in a sandbox or on a machine without Docker, I want the executor to keep a real execution path instead of hard-failing when Docker is unavailable, so I can still run commands and produce artifacts locally.
