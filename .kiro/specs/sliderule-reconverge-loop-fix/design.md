# SlideRule Reconverge Loop Fix Bugfix Design

## Overview

This is a follow-up bugfix to `sliderule-goal-conclusion-gate`. That spec made a converged SlideRule
V5.1 session (route `/sliderule`) reach `goal.status === "clear"`, written through the single-writer
...
Code review of `client/src/lib/sliderule-runtime.ts` confirmed two distinct defects, each modeled as
...
The fix strategy is minimal and surgical, scoped to `client/src/lib/sliderule-runtime.ts` plus tests:
...
`client/src/pages/SlideRule.tsx` already binds the STATUS badge to `sessionState.goal.status`, so it
needs no change. No server contract / socket / `/tasks` deep-link changes. The GCOV gate logic
itself is correct and must not be altered. The `sliderule-goal-conclusion-gate` invariants and the
`verify:sliderule-v5` closed-loop suite (63 client + 13 server) must stay green.
...
- **pickNextCapabilities**: The function in `client/src/lib/sliderule-runtime.ts` (~L72) that selects
...
- The GCOV-pass write path MUST be unchanged — GCOV continues to write `"clear"` / `"not_recommended"`
  exactly as in `sliderule-goal-conclusion-gate`.
...
- `SlideRule.tsx` continues to bind the STATUS badge to `sessionState.goal.status` (no change
  required).
...
4. **C-2: risk of introducing a second writer**: a naive fix could assign `goal.status` directly
   inside `invalidateForIntervention`, violating the `sliderule-goal-conclusion-gate` single-writer
...
- Full closed loop on `/sliderule`: drive a session to `goal.status === "clear"`, challenge the
...
- Regression: run the existing `sliderule-goal-conclusion-gate` tests and the `verify:sliderule-v5`
