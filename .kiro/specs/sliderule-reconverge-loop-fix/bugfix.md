...
This is a follow-up bugfix to `sliderule-goal-conclusion-gate`. That spec correctly added the
GCOV→GOAL conclusion write so a converged SlideRule V5.1 session (route `/sliderule`) can reach
...
Code review of `client/src/lib/sliderule-runtime.ts` confirmed two further defects that break the
...
Scope: `client/src/lib/sliderule-runtime.ts` (a stale-aware kind/gap presence check in
`pickNextCapabilities` for C-1; a single-writer goal downgrade on the invalidation path for C-2)
plus tests. `client/src/pages/SlideRule.tsx` already binds the STATUS badge to
...
`sliderule-goal-conclusion-gate` guarantees (single-writer `goal.status`, GCOV sole authority over
the GCOV-pass write, DERIVE P3 read-only) and the `verify:sliderule-v5` closed-loop suite (63 client
...
3.5 WHEN GCOV passes on the GCOV-gated path THEN the system SHALL CONTINUE TO write `goal.status` (`"clear"` / `"not_recommended"`) exactly as in `sliderule-goal-conclusion-gate`; the GCOV-pass write path is unchanged.
...
3.9 WHEN the STATUS bar renders THEN it SHALL CONTINUE TO bind to `sessionState.goal.status` (no change to `SlideRule.tsx` is required by this fix).

3.10 WHEN the existing `sliderule-goal-conclusion-gate` tests and the `verify:sliderule-v5` closed-loop suite (63 client + 13 server tests) run THEN they SHALL CONTINUE TO pass green.
