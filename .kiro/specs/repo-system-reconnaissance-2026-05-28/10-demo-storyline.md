...
`SlideRule` is **Mission as Task Autopilot**: the user states a destination, the system plans a route, executes the fleet, and replays evidence — all on top of the closed Mission Execution chain. Every storyline below tells the same loop from a different audience's vantage point.
...
**Headline**: *Other agent platforms ship a chat-loop and call it a day. SlideRule ships a closed loop with audit, lineage, and replay built into the runtime.*
