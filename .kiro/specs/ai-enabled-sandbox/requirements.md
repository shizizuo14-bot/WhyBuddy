...
SlideRule 的 Docker 容器执行器（Lobster Executor，L22 已完成）目前只能运行普通脚本任务，容器内无法调用 LLM API。本需求定义了为 Docker 容器注入 AI 能力所需的全部行为——包括 AI 基础镜像构建、凭证安全注入、容器内统一 AI 调用接口、多种 AI 任务类型支持、执行结果回调推送，以及凭证防泄露机制。
