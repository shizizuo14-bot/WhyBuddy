# WhyBuddy Stale-Set Monotonic Bugfix Design

## Overview

This bugfix makes the WhyBuddy session stale set **durable (monotonic) across challenges**.

Today, in `client/src/lib/whybuddy-runtime.ts`, `invalidateForIntervention` computes the
...
separate `whybuddy-artifact-health-predicate` spec.
...
- **`invalidateForIntervention`**: The re-entry engine function in
  `client/src/lib/whybuddy-runtime.ts` that, given a state and a `UserIntervention`, marks the
