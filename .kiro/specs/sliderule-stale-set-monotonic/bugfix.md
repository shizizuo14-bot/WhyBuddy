...
This bugfix targets finding #3 from the SlideRule V5.1 full-path acceptance suite
(`docs/V5.1-full-path-test-plan.md` §2 S4 "两圈半断言"): the session's stale set is
**not durable across challenges**.

In `client/src/lib/sliderule-runtime.ts`, `invalidateForIntervention` computes the
...
> upstreams) are handled in a **separate spec** (`sliderule-artifact-health-predicate`).
