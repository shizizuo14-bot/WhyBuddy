...
> H. 二次代码对账(2026-06-11):任务 1.1 改指 `client/src/lib/sliderule-runtime.ts`(BudgetPolicy 真实所在);需求 11 降为「保持不回退」(orchestrate.plan 归因已落地,B9 测试为证);移除不存在的 maxTokens

## Introduction

本需求文档定义 SlideRule V5.1 推演引擎在「LLM 自主路由 + 多步自主推演」方向上的**净增量**。

本文档曾针对一份过期快照编写,若按原样实现会**回退已落地子系统**。当前版本已与 `main` 上已落地代码对账,明确区分「已落地基线(本 spec 不重新实现、不得回退)」与「净增量(本 spec 真正交付)」:

- **R1(已落地)** —— 路由校验 / 失败降级 / DLEDGER `source` 标注(`server/sliderule/orchestrate-plan.ts` + `shared/blueprint/sliderule-plan-validation.ts`)。
- **R2(已落地)** —— 多 Agent 协商引擎(`server/sliderule/deliberation-exec-map.ts` 将 `counter.argue` / `critique.generate` / `rebuttal.resolve` / `synthesis.merge` 路由进 brainstorm mini-session,含真实 adjudicator)。
...
- **R2_Deliberation_Engine**:已落地的多 Agent 协商引擎,`server/sliderule/deliberation-exec-map.ts` 将协商类能力路由进 brainstorm mini-session,经真实 adjudicator(`createAdjudicator`)裁决,产出经 synthesizer 综合
...
- **BUDGET**:预算闸,as-built 字段为 `{ maxTurns: 30, maxCapabilityRunsPerTurn, maxCapabilityRunsPerSession: 120, maxRepeatPerCapability: 6 }`(定义于 `client/src/lib/sliderule-runtime.ts:69`,**无 maxTokens**);每消息循环上限为 Session_Driver 选项 `maxLoopsPerMessage`(缺省 3,不进 BudgetPolicy schema)【修订 E/H】
...
- **Turn_Route_Projection**:确定性的 turn 时间线投影(`shared/blueprint/sliderule-turn-route.ts` 的 `deriveTurnRoute`),从运行时记录的事实派生站点,零 LLM、零 Session_State 写入
...
| 基线 | 落地位置 | 本 spec 约束 |
| --- | --- | --- |
| R1 路由校验 / 失败降级 / DLEDGER source | `orchestrate-plan.ts`、`sliderule-plan-validation.ts` | 净增量挂在既有 `executeOrchestratePlan` / `validateProposedPlan` 接缝上 |
...
9. THE 每个派生 `loopTurnId` SHALL 按既有闸语义计入 `maxTurns`(+1/轮);`maxTurns` 由此升格为**会话级总循环安全阀**;其缺省值 30 SHALL **保持不变**(`sliderule-runtime.fullpath-budget.test.ts` 等不变量测试硬编码该值,改默认即违背本 spec 的不回退承诺;后续如需扩容,SHALL 在同一提交中显式更新相关不变量测试并记录决策,不得静默改);WHEN Orchestrator 因 `maxTurns` 停泊时,THE Session_Driver SHALL 将其映射为 `budget_exhausted` 停止【修订 E/H】
...
> **删除说明(保留编号占位以维持后续编号稳定)**:原需求 4 要求把 `counter.argue` 作为单一「批判性思考者」提示词执行。as-built 中 `server/sliderule/deliberation-exec-map.ts` 已把 `counter.argue`(连同 `critique.generate`、`rebuttal.resolve`、`synthesis.merge`)路由进 R2 brainstorm mini-session 协商引擎并经真实 adjudicator 裁决。若保留单提示词需求,会把真实协商回退为 LLM 角色扮演,**构成对 R2 的回退**。`counter.argue` 的行为约束统一由需求 8(R2 协商引擎复用)承载。
...
> 已落地(本需求**整体**保持不回退):`useSlideRuleSession.ts:361` 已以 `capabilityId: "orchestrate.plan"` 经 `recordCapabilityRunCost` 归因路由成本,既有 B9 测试断言其落入 costLedger;摘要压缩(仅 id/kind/summary)亦已落地。下列验收标准全部为**防回归**约束,对应任务为验证任务而非构建任务。
...
> 已落地(S9,本需求扩展不回退):`shared/blueprint/sliderule-turn-route.ts` 的 `deriveTurnRoute` 已从运行时事实派生 turn 站点,且为零 LLM、零 Session_State 写入;`assertRouteCopySanitized` 保证文案无禁用术语;`buildRouteSummary` 折叠态摘要与展开态站点 token 一致。
...
7. THE 多轮投影的**采集端与渲染端** SHALL 被本 spec 认领:`useSlideRuleSession` SHALL 按轮采集 round facts(planSelectedCount / planSource / planReason / parkReason 等);`TurnRouteTimeline` SHALL 渲染多轮序列;产品页渲染接线与任务 4.3 共用 F0.1 硬前置,dev 页可先行【修订 D】