...
> D. 第 7 节补采集端(`useSlideRuleSession` per-round facts)与渲染端(`TurnRouteTimeline` 多轮)设计
...
> H. 二次代码对账:BudgetPolicy 真实位于 `client/src/lib/sliderule-runtime.ts:69`,无 maxTokens;需求 11 已落地(useSlideRuleSession.ts:361 + B9 测试),降为防回归
...
- **改为**:四个 Deliberation_Capabilities `{counter.argue, critique.generate, rebuttal.resolve, synthesis.merge}` 继续运行在既有 R2 引擎 `server/sliderule/deliberation-exec-map.ts`(mini-session + `createAdjudicator` 真实裁决 + `BrainstormSynthesizer` 综合)。本设计只**消费并防回归**该路径,不新增第二条 brainstorm 路径。
...
- **保留 Mission/Workflow/Runtime 主干**:Session_Driver 属于运行时层(`client/src/lib/sliderule-runtime.ts` 与 `server/sliderule/*`),是 `orchestrateReasoningTurn` 之外的外层驱动,不触碰 Mission 编排器或十阶段工作流引擎。
...
页面 `useSlideRuleSession.runTurn()` 当前执行**单轮**:
...
```mermaid
flowchart TD
    subgraph Page["页面层 (useSlideRuleSession)"]
      RT["runTurn(userText)<br/>+ per-round routeFacts 采集 (需求 14.7)"]
    end

    subgraph Runtime["运行时层 (sliderule-runtime + server/sliderule)"]
...
新增运行时函数,承载再入循环。建议位置:`client/src/lib/sliderule-runtime.ts`(与 `orchestrateReasoningTurn` 同模块,复用其内部 helper),并在 `server/sliderule/` 暴露等价驱动以支持服务端测试。
...
复用 `server/sliderule/orchestrate-plan.ts` 的 `executeOrchestratePlan()`,仅补净增量。R1 已落地能力作为保留基线**不在本节重新设计**。
...
| 11.1 路由成本归因 | **已落地**:`useSlideRuleSession.ts:361` 经 `recordCapabilityRunCost` 以 `"orchestrate.plan"` 归因,B9 测试断言 | 防回归断言,不重新实现 |
...
四个 Deliberation_Capabilities `{counter.argue, critique.generate, rebuttal.resolve, synthesis.merge}` 继续运行在已落地的 `server/sliderule/deliberation-exec-map.ts`:
...
扩展 `shared/blueprint/sliderule-turn-route.ts` 的 `deriveTurnRoute`,把 Session_Driver 让单个 turn 跑 N 轮的事实如实投影,并认领采集端与渲染端【修订 D】。
...
**采集端(需求 14.7)【修订 D】**:`useSlideRuleSession` 在 Session_Driver 的每轮回调(或消费 `DriveReasoningResult.loops`)中按轮采集 `TurnRoundFacts`,与既有单轮 routeFacts 采集并存;`rounds` 仅在多轮发生时填充,单轮路径零变化。
...
```typescript
// BudgetPolicy:**不新增字段、不改缺省值**。as-built 定义于 client/src/lib/sliderule-runtime.ts:69:
...
- 复用项目既有 **Vitest + fast-check**(`shared/blueprint/__tests__/sliderule-plan-validation.test.ts`、`sliderule-turn-route` 测试已是同模式),不从零实现 PBT。
- 每个属性测试 **最少运行 100 次迭代**(`fc.assert(..., { numRuns: 100 })` 或默认)。
- 每个属性测试以注释标注其设计属性:标签格式 **Feature: sliderule-llm-autonomous-reasoning, Property {number}: {property_text}**
...
- 运行 `verify:sliderule-v5` 闭环套件与相关 `node --run check`,确保新增改动不扩大 TypeScript 历史类型债基线、不破坏既有 5140+ 测试与 sliderule fullpath 不变量(N1 直接 goal.status 写入守卫、N2 budget-before-pick 顺序、N4 legacy id 守卫),并保持 R2 / D1 / F1 既有测试全绿(防回归)。