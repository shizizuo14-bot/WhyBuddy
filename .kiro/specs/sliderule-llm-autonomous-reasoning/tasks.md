# Implementation Plan: SlideRule LLM 自主推演(净增量对账·评审修订版)
...
> H. 二次代码对账:1.1 改指 sliderule-runtime.ts(BudgetPolicy 真实所在);2.4 降为验证任务(orchestrate.plan 归因已落地,useSlideRuleSession.ts:361 + B9);移除 maxTokens 引用
...
- 每条属性测试以注释标签标注:`Feature: sliderule-llm-autonomous-reasoning, Property {n}: {text}`,**最少 100 次迭代**。
...
- [x] 1. 基础类型与确定性提供器
  - [x] 1.1 再入类型与既有预算字段对接【修订 E/H】
    - **BudgetPolicy 不动**:它定义于 `client/src/lib/sliderule-runtime.ts:69`(**不是** v5-reasoning-state.ts),as-built 字段 `{ maxTurns: 30, maxCapabilityRunsPerTurn, maxCapabilityRunsPerSession: 120, maxRepeatPerCapability: 6 }`,无 maxTokens;**不新增字段、不改缺省值**(maxTurns=30 等有不变量测试硬编码)
  ...
  - [x] 1.2 实现 Deterministic_Provider 装配与 BUILD_TARGET=test 默认
    ...
    - 在 `client/src/lib/sliderule-runtime.ts` 与 `server/sliderule/` 暴露等价装配接缝
...
- [x] 2. LLM_Router 净增量(routerModel / 覆盖率摘要 / 收敛信号 / 成本归因)
  - [x] 2.1 config.routerModel 解析与回退
    - 在 `server/sliderule/orchestrate-plan.ts` 的 `executeOrchestratePlan` 接缝上解析路由模型 = `config.routerModel ?? config.model`
  ...
  - [x]* 2.4 验证:orchestrate.plan 成本归因已落地(防回归)【修订 H】
    - **不重新实现**:`useSlideRuleSession.ts:361` 已经 `recordCapabilityRunCost(capabilityId: "orchestrate.plan")` 归因,既有 B9 测试断言其落入 costLedger
...
- [x] 4. Session_Driver 再入循环(核心净增量)
  - [x] 4.1 实现 driveReasoningSession 循环核心
    - 在 `client/src/lib/sliderule-runtime.ts` 实现外层驱动,反复调用 `router.proposePlan` → `orchestrateReasoningTurn(state, {proposedPlan})` → 逐能力提交;在 `server/sliderule/` 暴露等价驱动支持服务端测试
  ...
  - [x] 4.3 页面 runTurn 接入 Session_Driver(单一真相源)
    - **接线已完成(2026-06-11)**:`useSlideRuleSession.runTurn` → `driveReasoningSession` + `createServerReasoningRouter` + `createUiCapabilityExecutor`。F0.1 产品手验(llm 规划站)仍建议在 `dev:all` + `?executor=server-llm` 下补验。
    - `useSlideRuleSession.runTurn` 改为调用 `driveReasoningSession`,复用现有单能力提交语义作为 inner commit(Page_Commit_Path 保留为单轮提交原语,Session_Driver 为多步推进唯一所有者)
...
- [x] 7. S9 多轮 turn 时间线投影升级(投影 + 采集 + 渲染)
  - [x] 7.1 扩展 deriveTurnRoute 多轮投影
    - 在 `shared/blueprint/sliderule-turn-route.ts` 扩展 `deriveTurnRoute`:为每轮派生一对站点(planning + reasoning/execution),按 planning₁→reasoning₁→…→planningN→reasoningN 排列
  ...
  - [x] 7.2 多轮事实采集与时间线渲染接线【修订 D】
    ...
    - 采集端:`useSlideRuleSession` 消费 `DriveReasoningResult.loops`(或每轮回调)按轮采集 `TurnRoundFacts`(planSelectedCount / planSource / planReason / dledgerDecisionId / parkReason);单轮路径零变化,`rounds` 仅多轮时填充
...
- [x] 8. Checkpoint - 确保净增量实现层测试通过
  ...
  - **F0.1 仍阻断**:产品页 `/sliderule` 非关键词一轮「规划」站 `llm` + DLEDGER `source:"llm"` 未验收 → **4.3 / 7.2 跳过**。净增量 PBT(39 属性对应 `*` 子任务)与 fullpath vitest 全绿。
...
- [x] 10. 最终基线回归
  - [x]* 10.1 基线回归套件
    - 运行 `verify:sliderule-v5` 闭环套件与 `node --run check`
    - 断言:不扩大 TypeScript 历史类型债基线、不破坏既有 5140+ 测试、不破坏 sliderule fullpath 不变量(N1 直接 goal.status 写入守卫 / N2 budget-before-pick 顺序 / N4 legacy id 守卫),并保持 R2 / D1 / F1 既有测试全绿
    - _Requirements: 13.4_
...
- [x] 11. Final Checkpoint - 确保全部测试通过
  ...
  - `verify:sliderule-v5` + `node --run check` 全绿(2026-06-11)。**F0.1 代码侧已修**(alias 规范化 / convergence 透传 / store vitest);**产品页 llm 来源仍待 live LLM 手验** → 4.3/7.2 未接线。`origin/main`=`411a9296ad602b2dfd49f1187e326009ef6a30ea`。
...
- 39 条 Correctness Properties 各对应恰好一个 `*` 属性测试子任务,标签格式 `Feature: sliderule-llm-autonomous-reasoning, Property {n}: {text}`,最少 100 次迭代,零真实 LLM 调用。
