...
在 SlideRule 平台中新增完整的成本可观测性系统，实时追踪 LLM Token 消耗、费用预估、预算限制，并在 3D 办公室和 /tasks 页面中以可视化看板形式呈现。系统支持预警和自动降级策略，当 Token 或费用即将超限时自动发出预警并可切换低成本模型或暂停非关键 Agent。支持纯前端模式（本地模拟数据）和服务端模式双轨运行。
