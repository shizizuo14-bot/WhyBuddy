...
AR/VR 扩展模块为 SlideRule 3D 协作办公室提供沉浸式虚拟现实支持。用户通过 VR 头显进入 Agent 工作空间，以手势和空间交互操作任务看板、数据流和协作界面。该模块采用分层架构设计，预留通用接口支持多种 VR 平台（Meta Quest、HTC Vive、Apple Vision Pro 等）。当前阶段实现核心数据模型和渲染管道，VR 硬件集成作为远期演进方向。
