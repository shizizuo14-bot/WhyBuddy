# SlideRule GOAL Conclusion Gate Bugfix Design

## Overview

SlideRule V5.1 (route `/sliderule`) names the Coverage Gate (GCOV) as the single authority allowed
...
`goal.status = "needs_refinement"` and no path in `client/src/lib/sliderule-runtime.ts` or
`client/src/pages/SlideRule.tsx` ever transitions it. `evaluateCoverageGate` and the GCOV block in
...
and the closed-loop suite (63 client + 13 server tests, `verify:sliderule-v5`) must stay green.
...
- **ORCH**: `orchestrateReasoningTurn(state, context)` in `client/src/lib/sliderule-runtime.ts`. Must
  stay read-only on `goal` in its scheduling/budget/pick logic.
- **DERIVE**: `deriveNodeStatus(state)` in `client/src/lib/sliderule-runtime.ts`. A read-only
...
- The existing closed-loop suite (63 client + 13 server tests, `verify:sliderule-v5`) MUST stay green.
...
3. **STATUS bar bound to the wrong source**: `SlideRule.tsx` renders the local React `goal` string in
...
**File**: `client/src/pages/SlideRule.tsx`
...
3. **Coverage cannot be satisfied**: waive all blocking gaps but leave a required pre-req without a
   ...
   computed).
4. **STATUS bar binding** (edge case): render `SlideRule` with a GCOV-passed session and assert the
...
- Full `/sliderule` flow: ordinary turns → converge → GCOV-pass → STATUS bar shows `clear`.
- Hard-block flow: converge with missing pre-reqs → partial AWAIT, STATUS bar stays `needs_refinement`.
- `verify:sliderule-v5` closed-loop suite (63 client + 13 server tests) remains green after the fix.
