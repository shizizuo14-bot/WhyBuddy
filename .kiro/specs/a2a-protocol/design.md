...
A2A 协议在 SlideRule 现有的 MessageBus、动态组织生成、Mission Runtime 和 Guest Agent 机制基础上，新增跨框架 Agent 互操作层。该层包含三个核心模块：A2A 协议类型定义（共享层）、A2A Client（发起对外调用）和 A2A Server（接收外部调用），以及针对 CrewAI / LangGraph / Claude 的框架适配器。
