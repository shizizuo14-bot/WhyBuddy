# SlideRule Artifact-Health Predicate Unification Bugfix Design

## Overview

This bugfix unifies three call sites in `client/src/lib/sliderule-runtime.ts` onto a
...
The only production file expected to change is `client/src/lib/sliderule-runtime.ts`,
plus tests.

**Cross-dependency / ordering:** A separate spec `sliderule-stale-set-monotonic` makes
...
- **`staleArtifactIds`**: The authoritative session-level set of stale artifact ids,
  now monotonic per `sliderule-stale-set-monotonic`.
...
5. **Do not change anything else**: the GCOV / commit-gate write path, `applyGoalConclusion`
   ...
   (owned by `sliderule-stale-set-monotonic`) are untouched.
