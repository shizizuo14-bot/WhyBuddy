...
This plan unifies three call sites in `client/src/lib/sliderule-runtime.ts`
...
**Ordering dependency:** This spec MUST land AFTER `sliderule-stale-set-monotonic`, because
...
- [x] 1. Write bug condition exploration test
  ...
  - **GOAL**: Surface counterexamples that demonstrate the three-site disagreement in `client/src/lib/sliderule-runtime.ts` (`pickNextCapabilities` kind-presence vs `hasTrustedCommittedForCap` vs `findInputsForCapability`)
  ...
  - Mirror the V5.1 full-path S10 ("C-1 untrusted variant") and S4 ("challenging an UPSTREAM risk") `it.fails` assertions in `client/src/lib/sliderule-runtime.fullpath-budget.test.ts` so the exploratory test maps directly to the acceptance assertions
...
- [x] 3. Fix for three-site artifact-health disagreement (unify on a shared predicate)
  ...
  - [x] 3.1 Implement the shared predicate and apply it at all three sites
    ...
    - Do NOT change anything else: the GCOV / commit-gate write path, `applyGoalConclusion` single-writer downgrade, DERIVE read-only projection, and the stale-set accumulation (owned by `sliderule-stale-set-monotonic`) are untouched
...
- [x] 4. Checkpoint - Ensure all tests pass
  - Run the full relevant test suite (`sliderule-runtime` unit + property tests, plus the V5.1 full-path S10 and S4 integration flows) and confirm everything is green
  ...
  - Confirm the only production file changed is `client/src/lib/sliderule-runtime.ts`
...
- This spec depends on `sliderule-stale-set-monotonic` (fixed FIRST): the unified predicate
...
- Scope is limited to unifying the artifact-health predicate across the three call sites; the
  stale-set accumulation semantics are owned by `sliderule-stale-set-monotonic`.
