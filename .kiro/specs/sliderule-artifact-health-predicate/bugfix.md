...
Three call sites in `client/src/lib/sliderule-runtime.ts` decide whether a given
...
acceptance suite (`client/src/lib/sliderule-runtime.fullpath-budget.test.ts`):
...
`sliderule-stale-set-monotonic` (being fixed FIRST) changes stale-set accumulation
...
`client/src/lib/sliderule-runtime.ts` (plus tests).
...
`client/src/lib/sliderule-runtime.fullpath-budget.test.ts` (and the S4 case in the
