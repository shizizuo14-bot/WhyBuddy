...
`autopilot-image-rendering-and-visual-system` 是一个跨三阶段的合并式 mega-spec，覆盖 `SlideRule` 自动驾驶产品线在「effect preview 真实图像渲染」「Sora 风格视觉令牌系统」「项目主链进度时间线」三条主线上的统一交付。三个阶段在 DAG 中可并行执行，但 Phase 1 和 Phase 3 的可视化输出必须最终消费 Phase 2 提供的视觉令牌。
...
**User Story:** As a 服务端开发者, I want 服务端图像调用与 imageTest(2).html 已验证的协议完全一致, so that 不引入未验证的字段并复用 sliderule 已知的代理拓扑。
...
4. THE ImageService SHALL 通过 `client → sliderule server → image proxy domain → image model` 的链路完成调用，禁止浏览器直连图像模型域。
