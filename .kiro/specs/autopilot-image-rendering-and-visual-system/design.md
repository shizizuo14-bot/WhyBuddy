...
- raster 调用是 Stage C 的最末一步，也是唯一一处真正访问外部图像模型代理域的步骤。前三步都在 sliderule server 内部完成，浏览器侧不持有任何 image API 凭证。
...
```mermaid
graph TB
  subgraph Server["sliderule server (server/)"]
