...
在 SlideRule 平台已有的 cost-observability 模块（被动成本监控）基础上，演进为主动的成本治理系统。本模块提供多级预算管理、智能降级策略、并发/速率限制、任务暂停与人工审批、成本预测与优化建议、成本分摊与多租户管理、成本报表分析，以及与动态组织和权限模型的深度集成。目标是形成"预测 → 预算 → 监控 → 告警 → 响应 → 优化 → 审计"的完整成本治理闭环。
