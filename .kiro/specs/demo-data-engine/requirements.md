...
为 SlideRule 多智能体可视化教学平台构建预录演示数据引擎。该引擎提供一套完整的预录数据包，用于驱动前端完整的 Demo 体验流程，无需任何 LLM API Key 依赖。数据包以 TypeScript 模块形式存储，符合 shared/ 目录下的契约类型，支持序列化/反序列化以便未来录制新的演示数据。
