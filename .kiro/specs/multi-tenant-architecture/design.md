...
多租户架构为 SlideRule 平台引入完整的租户隔离能力，使平台从单用户演示模式演进为支持多团队/公司独立运营的商业化平台。设计采用逻辑隔离（Shared Database, Separate Schema）策略，在现有 JSON 文件存储基础上扩展租户维度，通过中间件层自动注入租户上下文，实现数据隔离、资源配额管理和基于角色的访问控制。
