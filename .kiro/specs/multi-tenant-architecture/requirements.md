...
多租户架构为 SlideRule 商业化平台提供完整的租户隔离方案。每个租户（团队/公司）拥有独立的 Agent 池、知识库、执行环境和资源配额，同时可共享公共的 Agent Marketplace。系统通过租户上下文传递、数据隔离、资源隔离和访问控制实现多租户支持，为平台从单用户演示走向多租户商业化运营奠定基础。
