...
本 spec 是 SlideRule autopilot blueprint 流系列 5 个 spec 中的**第三个**，建立在系列 spec 1（`autopilot-asset-staleness-model`，规划 `staleSince` / `invalidatedBy` 字段、依赖图、`invalidateDownstream` 引擎与 `GET /stale-artifacts` 端点）与系列 spec 2（`autopilot-replan-and-branch-action`，规划 `POST /replan` 端点与确认弹窗交互模式）之上。
