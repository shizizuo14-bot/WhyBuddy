# 端云 / SlideRule — Rebrand · Stage 3 Unblock · MiroFish Skin · README Refresh
...
"端云 / SlideRule" brand and unblock the only piece of the autopilot pipeline that
...
- The full 287-spec rename of any internal symbols (`mission`, `workflow`,
  `runtime`, `sliderule` strings inside steering files older than
...
### Task C — Brand alias (sliderule → 端云 / SlideRule)

**Source (user words)**: "整体项目不叫 cube-pet-office 了，改成了 sliderule，
端云… 因为我有这个域名 sliderule.com…端侧执行，云端调度的概念".

**Strategy**: alias-first, not big-bang rename.
- C.1 Add a `shared/brand.ts` constant module that exports
  `BRAND_NAME_DISPLAY = "端云"`, `BRAND_NAME_LATIN = "SlideRule"`,
  `BRAND_DOMAIN = "sliderule.com"`, `BRAND_TAGLINE_ZH = "端侧执行 · 云端调度"`,
  `BRAND_TAGLINE_EN = "Edge execution · Cloud orchestration"`,
  `BRAND_PACKAGE_LEGACY = "sliderule"`.
- C.2 Update user-visible touchpoints to consume the new brand:
  - `package.json` `name` → `sliderule`.
  - `client/index.html` `<title>` → `端云 SlideRule · 任务自动驾驶`.
  - `client/src/pages/auth/AuthPage.tsx` "SlideRule" header → brand
    constant.
  - `client/src/components/LoadingScreen.tsx` brand line → brand constant.
  - `README.md` H1 → `端云 / SlideRule`.
...
- D.1 `client/src/components/LoadingScreen.tsx` — replace the holographic
  ...
  font wordmark `端云 / SlideRule`, single accent stroke at `#FF4500`, no
...
- `feat(brand): introduce sliderule alias for visible touchpoints`
...
- `sliderule` literals inside the 287 specs and inside server-side
  audit / lineage event families. Routed to a future
  `sliderule-internal-rename-2026-XX-XX` spec.
...
- Lobster Executor inclusion in the compose file. Routed to a future
  `sliderule-compose-with-executor` spec.
- A full visual-regression Playwright suite. Routed to a future
  `sliderule-visual-regression-suite` spec.
