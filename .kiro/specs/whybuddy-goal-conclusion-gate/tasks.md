...
This plan executes the bugfix workflow for the WhyBuddy V5.1 GOAL conclusion gate using the
...
- **Wave 6 — Task 4**: all-greens checkpoint including `verify:whybuddy-v5`.
...
- [x] 1. Write bug condition exploration test
  ...
  - Add an edge-case rendering check: render `WhyBuddy` with a GCOV-passed `sessionState` and assert the STATUS bar conclusion badge shows the `clear` label sourced from `sessionState.goal.status`
  - Run test on UNFIXED code (`client/src/lib/whybuddy-runtime.ts`, `client/src/pages/WhyBuddy.tsx`)
...
- [x] 3. Fix for missing GCOV-gated GOAL conclusion write
  ...
  - [x] 3.1 Add pure conclusion-derivation and single-writer functions
    - Add pure `deriveGoalConclusion(state, gateResult, contract)` to `client/src/lib/whybuddy-runtime.ts` returning the next `goal.status`:
  ...
  - [x] 3.4 Surface the conclusion in the STATUS bar
    - In `client/src/pages/WhyBuddy.tsx`, add a conclusion badge bound to `sessionState.goal.status` next to the existing `目标` / `phase` indicators (labels: `clear` → 已收敛 / clear, `needs_refinement` → 待细化, `not_recommended` → 不建议)
...
- [x] 4. Checkpoint - Ensure all tests pass
  ...
  - Add integration tests: full `/whybuddy` flow ordinary turns → converge → GCOV-pass → STATUS bar shows `clear`; hard-block flow → partial AWAIT, STATUS bar stays `needs_refinement`
...
- **Compatibility-first**: no server contract changes, no socket changes, no `/tasks` deep-link
  changes; the only files touched are `client/src/lib/whybuddy-runtime.ts` and
  `client/src/pages/WhyBuddy.tsx` plus their tests.
- **Green gate**: `verify:whybuddy-v5` (63 client + 13 server tests) MUST stay green at Task 4;
