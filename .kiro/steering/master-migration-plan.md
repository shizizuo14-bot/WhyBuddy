...
# 总迁移计划：将 `web-aigc` 编排能力引入 SlideRule

## 目标

建立一套迁移方案，把 `web-aigc` 中可迁移的编排运行时能力引入 SlideRule，同时避免过早推翻 Cube 当前以 mission 为中心的运行架构。
...
- 将接口逐项映射到 `sliderule` 的 route / runtime / storage 落点
