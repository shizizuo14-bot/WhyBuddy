...
- `release-stability-guardrails-v2` 当前约 `85%`，已具备最小门禁、README/CI/脚本对齐与轻量关键链路 spot-check，但仍不应误判为“已具备完整 re-attach 闭环”
...
4. 并行推进 `release-stability-guardrails-v2`
   ...
   - 当前判断：统一门禁命令、CI / README / scripts 口径与轻量关键链路入口已对齐；完整 re-attach 仍待实现闭环
