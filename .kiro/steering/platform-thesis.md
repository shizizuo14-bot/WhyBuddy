...
# 平台主张：在 SlideRule 中构建图式 Agent 运行时

## 文档目的

本文用于说明，为什么 SlideRule 需要吸收 `web-aigc` 中部分 AI Agent 编排能力，以及为什么迁移目标应该是“分层平台能力”，而不是一次性的页面搬运或节点复制。

## 核心判断

SlideRule 目前已经具备比较强的运行时底座能力：
...
所以正确战略不是把整个 `web-aigc` 后台壳子照搬过来，而是让 SlideRule 变成一个“运行时优先、可承载图式编排”的 Agent Office 平台：
...
如果迁移方向正确，SlideRule 可以形成这样一种差异化定位：
