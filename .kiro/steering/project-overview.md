...
## 2026-05-28 Rebrand: 端云 / SlideRule

This project's user-facing brand has changed from `SlideRule` to
`端云 / SlideRule`. Strategy is **alias-first, not big-bang rename**:
...
- Internal symbols (file names, module identifiers, audit / lineage event
  ...
  their existing strings; a future `sliderule-internal-rename` spec will
...
- SlideRule 的产品口径已经从 `mission-first` 任务操作系统升级为 `task autopilot` 任务自动驾驶平台
...
一句话定义：`SlideRule` 是一个面向复杂任务的任务自动驾驶平台：用户输入目标、查看路线，让系统执行安全部分，并在人类判断必需时接管。

SlideRule 当前对内定义为：建立在 `mission-first` 底座上的 `task autopilot` 任务自动驾驶平台。
