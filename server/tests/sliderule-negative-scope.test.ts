...
import { isDeliberationCapability } from "../sliderule/deliberation-exec-map.js";
...
describe("Task 9.1: negative scope regression smoke", () => {
  ...
  it("does not introduce D_GATE capability whitelist in sliderule source", () => {
    const slideruleHits: string[] = [];
    for (const file of files) {
      const rel = relative(ROOT, file).replace(/\\/g, "/");
      if (!rel.includes("sliderule") && !rel.includes("SlideRule")) continue;
      const text = readFileSync(file, "utf8");
      if (/\bD_GATE\b/.test(text) && /WHITELIST|whitelist|capability.*gate/i.test(text)) {
        slideruleHits.push(rel);
      }
    }
    expect(slideruleHits).toEqual([]);
  });

  it("sliderule execute-capability does not route deliberation caps through wrapStageWithBrainstorm", () => {
    const routePath = join(ROOT, "server/routes/sliderule.ts");
