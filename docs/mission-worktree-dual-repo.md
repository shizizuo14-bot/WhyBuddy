...
这份约定用于支撑 `sliderule` 在多 `git worktree` 并行开发时，稳定参考 `openclaw-feishu-progress` 的已有实现，同时避免：

- 多个 worktree 同时争抢同一批目标文件
- `sliderule` 对兄弟仓库形成运行时依赖
- 迁移过程中把“参考代码”误当成“可直接复用的线上依赖”
- 在并行阶段过早改动 `server/index.ts`、README、环境变量文档等高冲突文件

本轮核心原则只有一句话：

`openclaw-feishu-progress` 只作为“读源仓库”，`sliderule` 才是“唯一落地仓库”。
...
- 仓库：`C:\Users\wangchunji\Documents\sliderule`
...
这样从任何一个 `sliderule-*` worktree 出发，都能用固定兄弟路径访问参考仓库：
...
- 只在 `sliderule` 的主仓或其 worktree 中提交代码；`openclaw-feishu-progress` 默认不提交任何改动。
- 禁止在 `sliderule` 代码中出现对 `..\openclaw-feishu-progress\**` 的运行时 `import`、动态加载、符号链接或包链接。
...
- 把核心类型、状态机、路由结构、UI 交互改写后落回 `sliderule`
- 参考它的测试案例，为 `sliderule` 新能力补测试
...
如果某段逻辑在 `openclaw-feishu-progress` 里“能跑”，但和 `sliderule` 当前的 Express、Socket.IO、3D 页面结构、动态组织主线不一致，那么优先“适配 Cube”，而不是“保留 OpenClaw 原样”。
