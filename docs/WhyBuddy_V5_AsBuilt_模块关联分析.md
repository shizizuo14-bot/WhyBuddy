# WhyBuddy V5 实际落地架构 · 模块关联分析（As-Built）

> **性质**：本文档画的是**代码现实**，不是设计叙事。每个节点是 plan 链（117 个快照、两个会话、约 17 小时）里被 `verify:whybuddy-v5` 验证过的真实模块，每条边是真实存在的调用或数据流。
>
...
| 模块 | 锚点 | 职责 | 状态 |
|---|---|---|---|
| `App.tsx` | isWhyBuddyLocation 判定 | AuthBootstrap / Bridge 在 WhyBuddy 路由 early return；chrome-free 不发 `/api/auth/me`，401 噪音根除 | ✅ |
| `pages/WhyBuddy.tsx` | 页面壳 | 挂载时 opt-in 真实 executor；节点点击 → 按 runId / artifactId 精确重入 | ✅ |
| `lib/whybuddy-runtime.ts` | 闭环核心 | 见下表，31 测试覆盖 | ✅ |
| `lib/whybuddy-http-store.ts` | store 适配器 | 调 4 个公开端点；服务端 5xx 持久化失败可感知 | ✅ |
...
| 模块 | 职责 | 状态 |
|---|---|---|
| `blueprint/whybuddy-report-builder.ts` | `buildStructuredReport`，9 段中文 evidence-grade schema；会话 2 从 client 抽出，**消除了 server 反向 import client runtime 的架构异味** | ✅ |

### 服务端 · server/routes

| 模块 | 职责 | 状态 |
|---|---|---|
| `whybuddy.ts` · `POST /execute-capability` | capabilityId 分发；与 /autopilot 同一 LLM 栈（`getAIConfig` + `callLLMJson`），API key 收归服务端；report schema 校验；opt-in；错误 → 4xx/5xx（行 269 catch） | ✅ |
| `whybuddy.ts` · 4 个公开会话端点 | GET / PUT / LIST / DELETE，**契约 100% 冻结，全程未动** | ✅ |
| `whybuddy.ts` · durable 层 | 内存 Map + 原子写 JSON；`flushToDisk(): boolean`；PUT 失败回滚 + 5xx（杜绝"看似已存实则丢失"）；测试专用 `POST __reload` | ✅ |
...
| 模块 | 职责 | 状态 |
|---|---|---|
| `data/whybuddy-sessions.json` | durable 落盘文件，已 gitignore；`__reload` 走 HTTP 验证真恢复（进程内假阳性已修） | ✅ |
| `docs/WhyBuddyV5闭环总图_完整版_修复闭环.md` | verbatim Findings 台账 + 每阶段执行记录 ≈ T_LEDGER 问责中枢的文档形态 | ✅ |

### 五层护栏 · `verify:whybuddy-v5`（package.json，一键串联）
...
| 3 | tsc 全量类型检查 | 横跨全部 |
| 4 | browser smoke **5/5** 流程（`whybuddy-browser-smoke.mjs`） | 页面闭环 + consoleErrors 无 401 |
| 5 | store smoke **9 步**含持久性（`whybuddy-store-api-smoke.mjs`） | PUT/GET/LIST/DELETE/404 + HTTP `__reload` 真恢复 |
...
当前实现的失效链只有一条：`invalidate → staleArtifactIds → deriveNodeStatus → 重入`。V5.1 担心的"两套重入并存"（INTERV 链 vs FB/RP 老回路）在已落地的 V5 runtime 中**只有新链存在**——FB/RP 属于旧 Autopilot 侧。P2 的实际工作量取决于旧回路在多大程度上已被引用，若 WhyBuddy 线确无 FB/RP 引用，则 P2 退化为"搜代码确认无残留 + 文档声明"。
...
| 4. 全系统仅一条回炉路径 | ✅ WhyBuddy 线事实成立，待 P2 确认无 FB/RP 残留 |
...
4. **文档追加 + 提交**：`feat(whybuddy): add MCP GitHub capability executor adapter`。
...
*依据：两个 plan 会话（2026-06-09 18:58 → 2026-06-10 12:11）共 117 个快照；V5.1 修复清单文档（whybuddy_v5_1.md）。所有行号与测试计数取自 plan 链中最后一次全绿验证记录。*
