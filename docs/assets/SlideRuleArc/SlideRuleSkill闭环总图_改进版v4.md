# SlideRule Skill 闭环总图（改进版 v4 · 出图可信层：真成功才算数）
...
## v4 这一版加了什么（相对 v3）
- **出图审计节点 `check_previews_real`（◆◆）**：出图后查 `provenance` —— 揪出「本地兜底占位冒充」「ok:true 却带 error 的假成功」「多张字节相同的复制充数」。**由用户自己跑，agent 改不了这一步**。
- **出图 gate 判定换根（◆◆）**：`finalize_previews` 不再看「目录里有没有 .png 文件」，只认「**本次真生成成功的张数**」；并识破复制充数、对 503 自动重试、对读超时不重试、**禁止任何本地兜底假图**。
- **审计进台账 + 回炉**：审计结果记入 `checks_ledger.json`；发现假图则打回重出。
- 一句话：**连「出图」这步想偷懒、糊弄一张假图，都被同一套「真跑 + 留痕 + 审计」的机制按住了。保下限，连图也不放过。**
