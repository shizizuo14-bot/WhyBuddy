# Issue: SlideRule `/respond` relay / provider injection diagnosis
...
```bash
curl -s -m 130 -X POST "http://127.0.0.1:3001/api/sliderule/respond" \
...
- `shared/blueprint/sliderule-narration-immunity.ts` — 身份折叠、领域锚定、开场劫持探测（不误杀「我建议/我认为」）
...
3. 配置：`SLIDERULE_NARRATION_BRAND_WORDS`（逗号分隔品牌词表，可选）。