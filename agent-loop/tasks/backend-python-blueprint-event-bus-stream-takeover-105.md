# Backend Python 105: Blueprint event bus and stream takeover

## Execution status
- Status: pending
- Goal: Move Blueprint event append/projection/SSE stream ownership to Python-first runtime with Node retaining only socket compatibility.
...
- `agent-loop/tasks/backend-python-blueprint-event-bus-stream-takeover-105.md`
- `server/routes/blueprint.ts`
- `server/routes/blueprint/**event**`
- `slide-rule-python/services/blueprint_event_bus_runtime.py`
...
- Previous 105 queue tasks 1-1 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Implement Python event append, replay cursor, stream projection, and error envelope.
2. Bridge Node socket/SSE consumers to Python event stream without owning event semantics.
3. Cover append/replay/stream timeout/cancel tests in Python and Node.
