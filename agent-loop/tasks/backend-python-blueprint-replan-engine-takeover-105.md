# Backend Python 105: Blueprint replan engine takeover

## Execution status
- Status: pending
- Goal: Make Python own Blueprint replan calculation and downstream invalidation decisions.
...
- `agent-loop/tasks/backend-python-blueprint-replan-engine-takeover-105.md`
- `server/routes/blueprint.ts`
- `server/routes/blueprint/**replan**`
- `slide-rule-python/services/blueprint_replan_runtime.py`
...
- Previous 105 queue tasks 1-3 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Move replan branch validation, conflict classification, invalidation plan, and explanation to Python.
2. Keep Node route as mapper/compat only.
3. Test accepted, conflict, stale, noop, and failure cases.
