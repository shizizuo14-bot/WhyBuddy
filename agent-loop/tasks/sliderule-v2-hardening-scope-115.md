...
- [ ] Document that 115 hardens the existing five legacy Skills, not AIGC.
- [ ] Record the six execution phases and the rule that each phase must be reviewable on its own.
- [ ] Document dirty-tree rules: no git add -A, no unrelated files, use worktree for execution.
