# Backend Python 105: Node route allowlist audit

## Execution status
- Status: pending
- Goal: Produce an executable allowlist of Node-retained routes and fail if migrated routes still own business logic in Node.
...
- `agent-loop/tasks/backend-python-node-route-allowlist-audit-105.md`
- `server/routes/**`
- `server/index.ts`
- `docs`
...
- Previous 105 queue tasks 1-44 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Add route inventory script or test for Python-owned vs Node-retained paths.
2. Mark every retained Node route with reason and owner.
3. Fail on unclassified Node business routes.
