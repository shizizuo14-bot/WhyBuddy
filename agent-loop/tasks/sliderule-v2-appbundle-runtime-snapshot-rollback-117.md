# SlideRule V2 Runtime 117: AppBundle runtime snapshot and rollback

## Execution status
- Status: PENDING
- Phase: 117.06-appbundle-runtime
- Goal: Generate app runtime snapshots and rollback plans from version-pinned Skill models.
- Required runtime symbols: `createAppBundleRuntimeSnapshot`, `planAppBundleRollback`, `APPBUNDLE_ROLLBACK_UNPINNED`
...
- `agent-loop/tasks/sliderule-v2-appbundle-runtime-snapshot-rollback-117.md`
...
- [ ] Add pure exported `createAppBundleRuntimeSnapshot(model, models)` that captures version pins, refs, publish gate evidence, and runtime closure hash inputs.
- [ ] Add pure exported `planAppBundleRollback(currentSnapshot, targetSnapshot)`.
- [ ] Rollback without pinned versions must return `APPBUNDLE_ROLLBACK_UNPINNED`.
...
- `node agent-loop/src/check-mojibake.js client/src/lib/skills/appbundle/appBundleSkill.ts client/src/lib/skills/appbundle/appBundleModel.ts client/src/lib/skills/appbundle/appBundleSkill.test.ts agent-loop/tasks/sliderule-v2-appbundle-runtime-snapshot-rollback-117.md`

## Acceptance criteria
- Snapshot is deterministic for the same models.
- Rollback plan identifies changed skill versions/refs.
- Unpinned rollback is blocked.
