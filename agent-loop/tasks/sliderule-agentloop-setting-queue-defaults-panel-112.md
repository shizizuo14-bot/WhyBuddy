# SlideRule AgentLoop 112.06: queue defaults panel

## Execution status
- Status: pending
- Goal: rebuild the Queue Defaults tab to match reference image 3 with current JSON, dry-run patch preview, copy controls, and apply safety messaging.
- Required gate: `slideruleAgentLoopSettingQueueDefaultsPanel112Gates`
...
## Context
Queue defaults are a product feature but can damage queue files if too loose. Keep the contract explicit: supported keys only, dry-run preview first, secrets/workerEnv rejected.
...
- `agent-loop/tasks/sliderule-agentloop-setting-queue-defaults-panel-112.md`
- `This task file`
...
## Acceptance criteria
- Add or update a test named `agentloop setting queue defaults 112 previews supported patch only`.
- Queue Defaults tab displays current defaults JSON with line-numbered code styling and a copy control.
- Preview patch section shows proposed JSON/diff dry-run output before apply.
- Sync-from-settings and structured-diff actions call the existing preview/apply bridge without inventing synthetic success.
- Unsupported keys, secrets, and workerEnv are visibly rejected or omitted with user-facing explanation.
