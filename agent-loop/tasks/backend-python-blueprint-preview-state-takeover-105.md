# Backend Python 105: Blueprint preview state takeover

## Execution status
- Status: pending
- Goal: Make Python own preview-state projection and mutation decisions consumed by the UI.
...
- `agent-loop/tasks/backend-python-blueprint-preview-state-takeover-105.md`
- `server/routes/blueprint/**preview**`
- `slide-rule-python/services/blueprint_preview_state_runtime.py`
...
- Previous 105 queue tasks 1-5 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Implement Python preview state read/write/projection with version guard.
2. Bridge Node preview endpoints to Python-first runtime.
3. Test stale version, invalid preview, accepted update, and degraded Python failures.
