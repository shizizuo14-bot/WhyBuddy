# Backend Python 105: Task mission durable store takeover

## Execution status
- Status: pending
- Goal: Move mission durable store ownership to Python for create/read/update/list lifecycle operations.
...
- `agent-loop/tasks/backend-python-task-mission-store-takeover-105.md`
...
- Previous 105 queue tasks 1-10 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Implement Python mission store runtime with durable-shaped adapter.
2. Delegate Node mission routes to Python-first store.
3. Test create/read/update/list, invalid state transitions, and fallback boundaries.
