# Backend Python 105: Auth session repository production takeover

## Execution status
- Status: pending
- Goal: Promote session repository from bounded slice to Python-owned production session lifecycle.
...
- `agent-loop/tasks/backend-python-auth-session-repository-production-takeover-105.md`
...
- Previous 105 queue tasks 1-16 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Implement Python session create/read/refresh/revoke as the source of truth.
2. Make Node cookie/session middleware consume Python session state.
3. Test login/me/refresh/logout end-to-end with Python session owner.
