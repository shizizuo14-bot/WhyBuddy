# Backend Python 105: Blueprint role runtime callback and tool takeover

## Execution status
- Status: pending
- Goal: Make Python own bounded role-agent runtime execution, callbacks, and tool result envelopes.
...
- `agent-loop/tasks/backend-python-blueprint-role-runtime-callback-takeover-105.md`
- `server/routes/blueprint/role-agent-runtime/**`
...
- Previous 105 queue tasks 1-8 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Implement Python role invocation lifecycle with callback progress and sanitized tool output.
2. Node keeps only route compatibility and request validation.
3. Test progress, callback, tool denial, timeout, and redaction.
