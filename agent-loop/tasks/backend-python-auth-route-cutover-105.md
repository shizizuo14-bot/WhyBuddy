# Backend Python 105: Auth route Python-first cutover

## Execution status
- Status: pending
- Goal: Cut /api/auth login/register/me/refresh/logout/email-code to Python-first while keeping Node cookie compatibility.
...
- `agent-loop/tasks/backend-python-auth-route-cutover-105.md`
...
- Previous 105 queue tasks 1-19 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Add Python auth facade for migrated auth routes.
2. Convert Node auth route handlers to proxy/mappers with explicit retained cookie behavior.
3. Run auth route tests proving Python receives the runtime operation.
