# Backend Python 105: Auth mailer production boundary takeover

## Execution status
- Status: pending
- Goal: Move email-code mailer boundary to Python or explicitly externalize it behind Python-owned provider config.
- Queue: `backend-python-total-cutover-105-queue`
- Phase: Auth Production Chain
- Sequence: 19 / 48
...
- `agent-loop/tasks/backend-python-auth-mailer-production-takeover-105.md`
...
- Previous 105 queue tasks 1-18 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Implement Python mailer provider contract with dry-run and production modes.
2. Route email-code send/verify flow through Python-first boundary.
3. Test no-key degraded state, dry-run, delivery success, and failure visibility.
