# Backend Python 105: Task route Python-first cutover

## Execution status
- Status: pending
- Goal: Cut /api/tasks and mission lifecycle high-traffic routes to Python-first and leave Node as thin shell.
...
- `agent-loop/tasks/backend-python-task-route-cutover-105.md`
...
- Previous 105 queue tasks 1-14 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Create Python task route facade for migrated mission/store/scheduler/auth surfaces.
2. Move Node route handlers to thin proxy or explicit retained allowlist.
3. Add route inventory assertion for Python-first task paths.
