# Backend Python 105: dev:all Python-first cutover

## Execution status
- Status: pending
- Goal: Make dev:all start and advertise Python-first backend behavior, with Node as compatibility server only.
...
- `agent-loop/tasks/backend-python-dev-all-python-first-105.md`
- `package.json`
- `scripts/dev-all.mjs`
...
- Previous 105 queue tasks 1-45 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Update dev startup order and health waits for Python service.
2. Print clear Python-first/Node-compat URLs and health checks.
3. Test dev script dry-run or smoke helpers where feasible.
