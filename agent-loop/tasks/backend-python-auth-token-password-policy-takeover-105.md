# Backend Python 105: Auth token and password policy takeover

## Execution status
- Status: pending
- Goal: Move token issuance/refresh/revoke and password policy validation to Python-owned runtime.
...
- `agent-loop/tasks/backend-python-auth-token-password-policy-takeover-105.md`
...
- Previous 105 queue tasks 1-17 when their outputs are relevant.
- Existing Python and Node tests near the files listed above

## Required implementation
1. Implement Python token lifecycle and password policy decisions.
2. Delegate Node token and password validation paths to Python-first service.
3. Test issue/refresh/revoke, weak password, expired token, and invalid token.
