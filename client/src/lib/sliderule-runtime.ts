/**
 * SlideRule V5 Thin Runtime (prototype, in-memory)
 *
 ...
 * - Designed so the SlideRule page can call these instead of local mocks.
 *
 * Key exports:
...
import { stripProjectionForPersist } from "@shared/blueprint/sliderule-projection-persist";
import {
  applyReplayOnSave,
  replaySessionEvents,
  type SlideRuleReplayEvent,
} from "@shared/blueprint/sliderule-session-replay";
import {
  ...
  type FragmentKind,
} from "@shared/blueprint/sliderule-report-builder.js";
...
} from "@shared/blueprint/sliderule-interactive-gates";
import {
...
import { shouldEscalateOnBudgetBlock } from "@shared/blueprint/sliderule-budget-esc";
import {
  applyRoleModeToState,
  isDeliberationCapability,
  markBrainstormDegraded,
} from "@shared/blueprint/sliderule-role-mode";
import { auditPreviewReal, isVisualIntent } from "@shared/blueprint/sliderule-visual-chain";
import {
...
// ===== V5.1 P4/B Budget Gate v1 (counts-based, minimal, per sliderule_v5.1.md) =====
...
// sliderule-llm-autonomous-reasoning 需求 1 / 2 / 3 / 13.
//
...
/** Product preview (/sliderule): Session_Driver keeps re-entering ORCH until AWAIT or loop cap. */
...
export type SlideRuleDriveMode = "single" | "marathon";
...
export function getPropositionRootNode(
  ...
  const sessionId = state.sessionId || "sliderule-local-proto";
...
export function createInitialSessionState(goalText: string, sessionId = "sliderule-local-proto"): V5SessionState {
 ...
 * interface (including a future HttpSlideRuleSessionStore) and be swapped in
 ...
 */
export interface SlideRuleSessionStore {
...
class InMemorySlideRuleSessionStore implements SlideRuleSessionStore {
...
let currentSlideRuleSessionStore: SlideRuleSessionStore = new InMemorySlideRuleSessionStore();

/**
 * For tests or future backend injection.
 * Swapping the store does not change the shape of load/save used by the page.
 */
export function setSlideRuleSessionStore(impl: SlideRuleSessionStore): void {
  currentSlideRuleSessionStore = impl;
}

export function getSlideRuleSessionStore(): SlideRuleSessionStore {
  return currentSlideRuleSessionStore;
}
 ...
 * Later this can be replaced by a real backend adapter implementing SlideRuleSessionStore
 * (e.g. fetch /api/sliderule/sessions/:id ) without changing any caller.
 */
export function isLegacyEmptySessionSeed(state: V5SessionState): boolean {
  const text = (state.goal?.text || "").trim();
  const isLegacyGoal =
    text === LEGACY_HARDCODED_PRODUCT_GOAL || text === "SlideRule V5 session";
...
export async function loadOrCreateSessionState(
  ...
  const existing = await currentSlideRuleSessionStore.load(sessionId);
  if (existing) {
...
export async function saveSessionState(state: V5SessionState): Promise<V5SessionState> {
  const sessionId = state.sessionId || "sliderule-local-proto";
  const previous = await currentSlideRuleSessionStore.load(sessionId);
...
export { replaySessionEvents, type SlideRuleReplayEvent };

export async function loadSessionReplayEvents(sessionId: string): Promise<SlideRuleReplayEvent[]> {
  const state = await currentSlideRuleSessionStore.load(sessionId);
  if (!state) return [];
  return replaySessionEvents(state);
}

export function clearSlideRuleSessionStore(): void {
  currentSlideRuleSessionStore.clear?.();
}

export function listSlideRuleSessions() {
  const res = currentSlideRuleSessionStore.listSessions?.();
  return res instanceof Promise ? res : (res || []);
}

export async function deleteSlideRuleSession(sessionId: string): Promise<void> {
  await currentSlideRuleSessionStore.deleteSession?.(sessionId);
}

/**
 * deriveNodeStatus — 状态派生（单一真相）
 *
 * 按 SlideRuleV5闭环总图 的 RUNTIME 子图：
 *   "DERIVE 实时进度 + 已存 → 单一真相"
 *
...
// Bugfix spec: sliderule-goal-conclusion-gate.
 ...
 * - verify:sliderule-v5 is now closer to hermetic (browser smoke can auto-spawn Vite)
 ...
 * - Module default is DefaultCapabilityExecutor. The /sliderule page effect opts the demo into PilotRealCapabilityExecutor for richer outputs during the pilot phase.
 *
 ...
 * Tests and the /sliderule page (demo) can opt-in to richer pilot outputs for risk.analyze + report.write.
 * The module default executor is DefaultCapabilityExecutor. The /sliderule page effect may opt the demo into PilotRealCapabilityExecutor.
 ...
 * the project's server LLM stack (`/api/sliderule/execute-capability` + getAIConfig + callLLMJson).
 ...
 *
 * The client only does a POST to the local backend (`/api/sliderule/execute-capability`).
...
export function createServerLlmCapabilityProvider(opts: { endpoint?: string } = {}): LlmCapabilityProvider {
  const url = opts.endpoint || "/api/sliderule/execute-capability";
 ...
 * your own backend at /api/sliderule/execute-capability, which uses the real server stack).
 *
...
// ============================================================================
// Deterministic_Provider assembly (需求 13 — sliderule-llm-autonomous-reasoning)
//
// Compatibility-first / additive only:
...
export function orchestrateReasoningTurn(
  ...
  // Bugfix spec: sliderule-goal-conclusion-gate (Task 3.2).
...
// ===== Session_Driver: multi-step re-entry loop (sliderule-llm-autonomous-reasoning, 需求 1 / 2) =====
...
// and any external code doing `import { buildStructuredReport } from './sliderule-runtime'`
