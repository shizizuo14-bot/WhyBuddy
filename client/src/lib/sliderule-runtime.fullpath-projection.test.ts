/**
 * SlideRule V5.1 Full-Path Acceptance Test Plan — Batch 3a: runtime projection & persistence.
 ...
 * judgement.
 *
 * STORE NOTE: `InMemorySlideRuleSessionStore` is NOT exported from sliderule-runtime.ts (only the
 ...
 * createdAt / lastActive / listSessions shape), via setSlideRuleSessionStore in beforeEach and
...
import {
  ...
  listSlideRuleSessions,
  deleteSlideRuleSession,
...
import { replayEventsBelongToSession } from '@shared/blueprint/sliderule-session-replay';
import { assertDeriveReadOnly } from './sliderule-derive-readonly-guard';
import { buildClearStateWithTrustedReport, COMPLEX_GOAL_TEXT } from './sliderule-fullpath-fixtures';
import { persistedStateHasNodeStatus } from '@shared/blueprint/sliderule-projection-persist';
import type { V5SessionState, UserIntervention } from '@shared/blueprint/v5-reasoning-state';

// ---------------------------------------------------------------------------------------
// Local in-memory store mirroring the runtime's (unexported) InMemorySlideRuleSessionStore.
// Same module-level Map + meta semantics so the durable-store contract is exercised faithfully.
// ---------------------------------------------------------------------------------------
class TestInMemorySessionStore implements SlideRuleSessionStore {
...
class TestSharedDurableSessionStore implements SlideRuleSessionStore {
  async load(sessionId: string): Promise<V5SessionState | undefined> {
...
let originalStore: SlideRuleSessionStore;

beforeEach(() => {
  originalStore = getSlideRuleSessionStore();
  clearSharedDurableBacking();
  setSlideRuleSessionStore(new TestInMemorySessionStore());
});

afterEach(() => {
  setSlideRuleSessionStore(originalStore);
});
...
describe('S21 · runtime projection & replay', () => {
  ...
  it('Replay isolation by sessionId: loading session A never surfaces session B events/artifacts', async () => {
    ...
    // listSlideRuleSessions surfaces both, keyed by sessionId.
    const list = await listSlideRuleSessions();
  ...
  it('P3 residual: the persisted (saved) state should NOT carry the node-status projection', async () => {
    ...
    const persisted = await getSlideRuleSessionStore().load('S21-residual');
...
describe('S22 · persistence & session isolation', () => {
  ...
  it('deleteSlideRuleSession removes only the targeted session (isolation on delete)', async () => {
    ...
    await deleteSlideRuleSession('S22-del-A');

    const ids = (await listSlideRuleSessions()).map((e: any) => e.sessionId);
  ...
  it('refresh persistence: a session saved before "refresh" should be recoverable after a fresh store instance', async () => {
    ...
    setSlideRuleSessionStore(createSharedDurableStore());
    await saveSessionState(state);
