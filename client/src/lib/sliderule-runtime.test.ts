...
import { isTestHelperEnabled } from '../../../server/routes/sliderule.ts';
import type { V5SessionState, Artifact, UserIntervention } from '@shared/blueprint/v5-reasoning-state';
import type { V5CapabilityId } from '@shared/blueprint/contracts';
import type { ControlSignal } from './sliderule-runtime';
import { loadByokPool, saveByokPool, clearByokPool, maskKey, validateByokPool } from './sliderule-byok-config';
import {
...
describe('sliderule-runtime V5 closed loop (behavioral regression)', () => {
  ...
  it('listSessions + deleteSession work on the store and are isolated', async () => {
    ...
    const listedRaw = listSlideRuleSessions ? listSlideRuleSessions() : [];
    ...
    if (deleteSlideRuleSession) await deleteSlideRuleSession('list-s1');
    const listedAfterRaw = listSlideRuleSessions ? listSlideRuleSessions() : [];
  ...
  it('SlideRuleSessionStore is swappable and public load/save functions delegate to the injected impl', async () => {
    ...
    const fakeImpl: SlideRuleSessionStore = {
    ...
    const prev = getSlideRuleSessionStore();
    try {
      setSlideRuleSessionStore(fakeImpl);
      ...
      setSlideRuleSessionStore(prev); // restore
      clearSlideRuleSessionStore();
    }
  });
  ...
  it('B2 BYOK config: save/load/clear/validate/mask roundtrip and security', () => {
    ...
    // the whole verify:sliderule-v5 chain. Install a minimal in-memory stand-in just for this it.
  ...
  it('LlmCapabilityExecutor supports injectable LlmCapabilityProvider (success, failure fallback, non-target cap)', async () => {
    ...
    try {
      ...
      // Server-routed provider (recommended production-aligned path via /api/sliderule/execute-capability).
  ...
  it('test helper routes (__clear / __reload) are disabled under production (production guard)', async () => {
    ...
    const originalFlag = process.env.SLIDERULE_ENABLE_TEST_HELPERS;
    try {
      // Use the real exported implementation from server/routes/sliderule.ts
      process.env.NODE_ENV = 'production';
      delete process.env.SLIDERULE_ENABLE_TEST_HELPERS;
      ...
      vi.resetModules();
      const routeMod = await import('../../../server/routes/sliderule.ts');
      ...
      app.use('/api/sliderule', prodRouter);
      ...
      if (originalFlag !== undefined) {
        process.env.SLIDERULE_ENABLE_TEST_HELPERS = originalFlag;
      } else {
        delete process.env.SLIDERULE_ENABLE_TEST_HELPERS;
      }
