/**
 * SlideRule V5.1 Full-Path — S16/S17 ROLES layer (brainstorm debate chain + degraded fallback).
...
describe("S17 · brainstorm degraded fallback", () => {
  const origDegradeEnv = process.env.SLIDERULE_BRAINSTORM_DEGRADE;

  afterEach(() => {
    if (origDegradeEnv === undefined) delete process.env.SLIDERULE_BRAINSTORM_DEGRADE;
    else process.env.SLIDERULE_BRAINSTORM_DEGRADE = origDegradeEnv;
  });
  ...
  it("D_DEG→D_SA: SLIDERULE_BRAINSTORM_DEGRADE=1 skips D_BO primers", () => {
    process.env.SLIDERULE_BRAINSTORM_DEGRADE = "1";
    const picks = pickNextCapabilities(
